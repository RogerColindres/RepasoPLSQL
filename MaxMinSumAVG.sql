/*Funciones de agrupación MAX,MIN,SUM,AVG*/
...
create table empleados(
 ...
 insert into empleados
  ...
  select Max(sueldo) SueldoMayor from empleados;
  
  select min(sueldo) MenorSueldo from empleados;
  
  select Max(sueldo) SueldoMayor, min(sueldo) MenorSueldo from empleados;
  
  select max(cantidadhijos) as Hijos from empleados where nombre like '%Perez%';
  
  select avg(sueldo) PromedioSueldo from Empleados;
  
  select avg(sueldo) PromedioSecretarias from Empleados 
  where seccion like '%Secretaria%';  
  
  select sum(sueldo) CostoSueldo from empleados;
  
  select sum(sueldo) CostoSueldo from empleados where seccion like '%Sistemas%';
  
  select * from empleados;
  
  select min(fechaingreso) FechaMenorIngreso, max(fechaingreso) FechaMayorIngreso 
  from Empleados; 